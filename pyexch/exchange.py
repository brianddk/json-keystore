...
class CbV3Auth(AuthBase):
    def __init__(self, api_key, secret_key, token_2fa=None):
        self.api_key = api_key
        self.secret_key = secret_key
        self.token_2fa = token_2fa

    def __call__(self, request):
        jwt_uri = jwt_generator.format_jwt_uri(request.method, request.path_url)
        jwt_token = jwt_generator.build_rest_jwt(jwt_uri, self.api_key, self.secret_key)        
...
class Coinbase(Exchange):
    ...
    def __init__(self, keystore):
        ...
        if keystore.get("default") == "coinbase.v2_api" and keystore.get("coinbase.v2_api.key") and keystore.get("coinbase.v2_api.secret"):
            ...
            self.v3_req_auth = CbV3Auth(
                keystore.get("coinbase.v3_api.key"),
                keystore.get("coinbase.v3_api.secret"),
            )
    ...
    def get(self, uri, params=None):
        ...
        if self.keystore.get("default") == "coinbase.oauth2":
            ...
            self._response = requests.get(self.trim_api + uri, auth=self.v3_req_auth, params=params)
        else:
    ...
    def post(self, uri, params=None):
        data = params
        self._response = cb_2fa_token = None
        uri = trim(uri)
        if data:
            self._params = params = data_toDict(data)
        if params and params.get('cb_2fa_token'):
            cb_2fa_token = params.get('cb_2fa_token')
            del params['cb_2fa_token']
        if self.keystore.get("default") == "coinbase.oauth2":
            if trim_cmp(uri, self.keystore.get("coinbase.oauth2.token_url")):
                ...
                if cb_2fa_token:
                    ...
                    auth = CbOa2Auth(self.keystore.get("coinbase.oauth2.token"), cb_2fa_token)
            ...
            if cb_2fa_token:
                uri = self.trim_api + uri
                auth = CbV3Auth(
                    self.keystore.get("coinbase.v3_api.key"),
                    self.keystore.get("coinbase.v3_api.secret"),
                    cb_2fa_token,
                )
                self._response = requests.post(uri, auth=auth, json=data)
            else:                
